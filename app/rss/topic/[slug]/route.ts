...
<title>EDICTO — Topic ${slug} statements</title>
<link>${process.env.NEXT_PUBLIC_BASE_URL || ""}/topics/${slug}</link>
...
function emptyFeed(slug: string) {
  return `<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel><title>EDICTO — Topic ${slug}</title></channel></rss>`;
}
